<template>
    <div>
        ...
        <Link v-if="!isLogout && link" class="menu-item"
            :class="[menuItemClass, { active: isActive || isAnySubmenuActive }]" :href="link || '#'"
            @click="handleClick">
        <div class="icon">
            <img :src="icon" alt="icon" />
        </div>
        <div class="label">{{ label }}</div>
        <div v-if="submenuItems && submenuItems.length > 0" class="right-icon"
