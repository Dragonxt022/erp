          ...
          <div v-if="showNotifications" class="notification-tray">
            ...
            <div class="notification-list">
              ...
              <div
                ...
                :class="{ unread: notification.lida === 0 }"
                @click="markAsRead(notification.id)"
              >
                <div class="notification-icon-type">
                  <svg
                    v-if="notification.tipo === 'info'"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <svg
                    v-else-if="notification.tipo === 'alerta'"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
...
const unreadNotifications = computed(() => {
  return notifications.value.filter((notification) => notification.lida === 0)
    .length;
});
...
onMounted(() => {
  ...
  checkNotificationsInterval = setInterval(() => {
    if (!showNotifications.value) {
      // Só busca se o painel estiver fechado
      fetchNotifications();
    }
  }, 300000); // 5 minutos
});
